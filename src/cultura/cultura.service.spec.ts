    ...
    beforeEach(async () => {
      ...
      const seedDatabase = async () => {
        ...
        it('findOne retorna una cultura por id', async () => {
            ...
            const cultura: CulturaEntity = await service.findOne( culturaAlmacenada.codigo);
        ...
        it('Crear una nueva cultura', async () => {
            ...
            const culturaAlmacenada: CulturaEntity = await repository.findOne({where: {codigo: newCulture.codigo}})
        ...
        it('Actualizar una cultura', async () => {
            ...
            const updatedCulture: CulturaEntity = await service.update(culture.codigo, culture);
            expect(updatedCulture).not.toBeNull();
          
            const culturaAlmacenada: CulturaEntity = await repository.findOne({ where: { codigo: culture.codigo } })
            expect(culturaAlmacenada).not.toBeNull();
        ...
        it('Excepción por la actualización de una cultura no valida', async () => {
            ...
            culture = {
              ...culture, nombre: "Nueva Cultura", descripcion: "Nueva descripcion de una cultura"
            }
        ...
        it('Borrar una cultura', async () => {
            ...
            const deletedCulture: CulturaEntity = await repository.findOne({ where: { codigo: culture.codigo } })
